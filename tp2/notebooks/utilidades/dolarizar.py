...
def obtener_precios():
    ...
    for i in range(len(fechas_pos)):
        if not esta(fechas_pos[i], cotizacion):
            if i > 2:
                cotizacion[ymd(fechas_pos[i])] = cotizacion_ant(
                    fechas_pos, i, cotizacion
                )
            else:
                cotizacion[ymd(fechas_pos[i])] = cotizacion_pos(
                    fechas_pos, i, cotizacion
                )
    return cotizacion
...
def dolarizar(df):
    ...
    df["precio"] = df.apply(lambda x: x["precio"] /
                            valor_dolar[(x["anio"], x["mes"], x["dia"])], axis=1)
    return df
...
def agregar_dolar_como_feature(df):
    valor_dolar = obtener_precios()
    df["valor_dolar"] = df.apply(
        lambda x: valor_dolar[(x["anio"], x["mes"], x["dia"])], axis=1
    )
    return df
